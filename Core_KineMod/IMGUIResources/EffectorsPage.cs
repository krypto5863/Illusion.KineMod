...
namespace Core_KineMod.IMGUIResources
{
	internal static class EffectorsPage
	{
		...
		{
			var effectors =
				character.finalIK.solver.effectors.GroupBy(m => character.finalIK.solver.chain[m.chainIndex]);
			...
			{
				var limbName =
					BonesToLimbName.TryGetValue(chainEffector.FirstOrDefault()?.target?.name.ToLower() ?? string.Empty,
						out var boneName)
						? boneName
						: string.Empty;
				...
				foreach (var effector in chainEffector.OrderBy(m => m.nodeIndex))
				{
					...
					{
						var displayName =
							(BonesUserFriendlyNames.TryGetValue(effector.target.name.ToLower(), out var newName))
								? newName
								: effector.target.name;
				...
				var displayName1 =
					(BonesUserFriendlyNames.TryGetValue(chainEffector.Key.bendConstraint.bendGoal.name.ToLower(),
						out var newName1))
						? newName1
						: chainEffector.Key.bendConstraint.bendGoal.name;
				GUILayout.BeginHorizontal();
				{
